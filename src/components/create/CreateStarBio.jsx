...
const CreateAlbums = () => {
    ...
    const [starnameOptions, setStarnameOptions] = useState([]);
    ...
    const fetchStarNames = async () => {
        try {
            ...
            setStarnameOptions(response.data.map(star => ({ value: star.name, label: star.name })));
        } catch (error) {
    ...
    const handleFileChange = (fileList) => {
        setFiles(fileList); // Update files directly instead of spreading
    };
    ...
    const props = {
        name: 'file',
        multiple: true,
        action: 'https://stardb-api.onrender.com/api/stars/albums/upload', // Change this to your upload endpoint
        onChange(info) {
